...
- lookup_snippet() fills in Google News items, whose feed text is just the headline: it searches
  Bing News RSS for the headline and uses the lead text of the matching story.
...
def clean_summary(text: str, title: str, source: str = "") -> str:
    ...
    if similarity(text, title) > 0.75 or text.lower().startswith(title.lower()[:60]):
        ...
        if not rest or rest.lower() == source.lower() or len(rest.split()) <= 5:
            return ""
    out = []
